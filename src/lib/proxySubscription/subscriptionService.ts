...
async function syncSubscriptionUnsafe(id: string): Promise<SyncResult> {
  ...
  // Keep the embedded proxy core (mihomo) in step with the subscription set:
  // start/restart/stop it as needs-core subscriptions appear, change, or vanish.
  // Dynamic import avoids a cycle (proxyCore/manager imports this module).
  // Best-effort and never fatal — a sync must not fail because the core is down.
  import("@/lib/proxyCore/manager")
    .then((m) => m.refreshProxyCore())
    .catch(() => {
      /* embedded core unavailable — non-fatal */
    });
